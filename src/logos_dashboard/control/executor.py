...
from . import governor, packet as pk, queue, runs, service, worktree
...
class ExecutorConfig:
    def __init__(self, repo: Path, worktrees_root: Path, runner_factory: Callable[[Path], Callable], *, regs_loader: Callable[[], dict], cli_version: str | None = None, limits: dict | None = None, keep_worktree: bool = True):
...
def run_job(conn, job: dict, cfg: ExecutorConfig) -> dict:
    ...
    # -- worktree -------------------------------------------------------------------------------------------------
    runs.emit(conn, run_id, "phase", {"phase": "worktree"})
    try:
    ...
    # -- packet ---------------------------------------------------------------------------------------------------
    runs.emit(conn, run_id, "phase", {"phase": "packet"})
    ...
    if queue.stop_requested(conn, job_id):
        runs.emit(conn, run_id, "stop", {"before": "invoke"}); runs.finish_run(conn, run_id, "stopped", "founder_stop"); _cleanup(cfg, wt, branch, discard=True)
    ...
    runs.emit(conn, run_id, "phase", {"phase": "claude"})
    ...
    try:
        ...
        _cleanup(cfg, wt, branch, discard=True)
    ...
    # -- outputs --------------------------------------------------------------------------------------------------
    runs.emit(conn, run_id, "phase", {"phase": "verify"})
