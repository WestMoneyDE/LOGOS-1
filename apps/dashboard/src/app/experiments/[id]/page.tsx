...
export default async function ExperimentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows: [string, React.ReactNode][] = [["order", e.order_id], ["verdict", <VerdictBadge key="v" verdict={e.verdict} />], ["kind", <KindBadge key="k" kind={e.kind} />], ["question", e.scientific_question], ["hypothesis", e.hypothesis],
    ...
    ["prereg hash", <code key="p" className="break-all font-mono text-xs">{e.prereg_hash || "— (deterministic order: prereg recorded in the closure)"}</code>], ["run id", <code key="r" className="font-mono text-xs">{e.run_id || "—"}</code>], ["artifact hash", <code key="a" className="break-all font-mono text-xs">{e.artifact_hash || "—"}</code>],
