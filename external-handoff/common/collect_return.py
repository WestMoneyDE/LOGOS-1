...
def main():
    ...
    envelope={"schema":"logos-external-return-v1","track":args.track,"created_at":datetime.now(timezone.utc).isoformat(),"scientific_ceiling":cfg["scientific_ceiling"],"source_pins":cfg["source_pins"],"files":[]}
    ...
    d=sha256(args.output)
    args.output.with_suffix(args.output.suffix+".sha256").write_text(f"{d}  {args.output.name}\n")
    print(json.dumps({"output":str(args.output),"sha256":d,"files":len(selected)},indent=2))
