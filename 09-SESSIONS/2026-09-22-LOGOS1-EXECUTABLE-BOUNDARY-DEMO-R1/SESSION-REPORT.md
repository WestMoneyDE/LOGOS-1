...
`core/governance.py` ist **keine zweite Implementierung**. Es enthält keine Regel, keine Ausnahme, keinen Override; jedes Urteil kommt aus `logos_gamma.validate`. Wird die Datei gelöscht, ändert sich am Verhalten von LOGOS-1 nichts. Ein zweites Γ-Regelsystem wäre ein Defekt, kein Feature — der Differenztest hält das fest. `core` ist zudem aus dem Distributionspaket ausgeschlossen (`pyproject.toml`): `core` ist ein zu generischer Name, um ihn in eine Umgebung zu installieren.
