...
def test_repository_orders_import_idempotent_and_chained(conn):
    ...
    assert len(docs) == 47 and sum(1 for d in docs if d["kind"] == "closure") == 42 and {d["state"] for d in docs} == {"VALIDATED", "FALSIFIED", "DRAFT"}     # 48 files, 47 orders (one generated master order has its own closure) — exact for 05-WORK-ORDERS on 2026-09-24 (+ LAYA-JUROR-INTEGRATION-R1 closure, + QUEUED-LOGOS1-LAYA-CALIBRATION-DATASET-R1; was 45/41 on 2026-09-22)
    ...
    assert r1["documents"] == r2["documents"] == 47 and r2["new"] == 0 and r2["updated"] == 47 and r2["edges_added"] == 0 and r1["states"]["FALSIFIED"] == 2
    ch = repo_orders.chain(conn); assert len(ch) == 47 and all(x["work_order_id"].startswith("REPO:") for x in ch) and all(x["origin"]["repository"] for x in ch)
    with conn.cursor() as cur:
...
def test_r2_api_autopilot_workers_repo_orders_leitstand(conn, tid, attested, autopilot_clean):
    ...
    ro = client.post("/api/ros/repo-orders/import").json(); assert ro["documents"] == 47
    lst = client.get("/api/ros/repo-orders").json(); assert len(lst["orders"]) == 47 and len(lst["documents"]) == 47
